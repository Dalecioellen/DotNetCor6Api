   ...
   public sealed class Person // sealed garante que ninguem herde dessa classe
    {
        ...
        public void Validation(string name, string document, string phone)
        {
            DomainValidationException.TemErro(string.IsNullOrEmpty(name), "O nome é obrigatório e não pode ser vazio");

            DomainValidationException.TemErro(string.IsNullOrEmpty(document), "O documento é obrigatório e não pode ser vazio");

            DomainValidationException.TemErro(string.IsNullOrEmpty(phone), "O telefone é obrigatório e não pode ser vazio");
